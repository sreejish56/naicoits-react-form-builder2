...
var Preview = /*#__PURE__*/function (_React$Component) {
  ...
  function Preview(props) {
    ...
    _this._onClone = _this._onClone.bind((0, _assertThisInitialized2["default"])(_this));
  ...
  (0, _createClass2["default"])(Preview, [{
    ...
    value: function _onChange(data) {
      ...
      data.forEach(function (item) {
        if (item && item !== null && item !== void 0 && item.readOnly && item !== null && item !== void 0 && item.variableKey && _this3.props.variables[item.variableKey]) {
    ...
    key: "_onClone",
    value: function _onClone(item) {
      var _this5 = this;

      var data = this.state.data;
      var hoverIndex = data.findIndex(function (row) {
        return row.id === item.id;
      }) + 1;

      var newItem = _objectSpread({}, item);

      newItem.id = _UUID["default"].uuid();

      if (typeof newItem.field_name !== "undefined") {
        newItem.field_name = newItem.field_name.replace(/[A-Z0-9\-]+/g, "") + _UUID["default"].uuid();
      }

      if (typeof newItem.options !== "undefined") {
        newItem.options = newItem.options.map(function (row) {
          var option = _objectSpread({}, row);

          option.key = option.key.replace(/[A-Z0-9\-]+/g, "") + _UUID["default"].uuid();
          return option;
        });
      }

      if (typeof newItem.childItems !== "undefined") {
        newItem.childItems = newItem.childItems.map(function (row, index) {
          var child = _this5.getDataById(row);

          if (child) {
            var newChild = _objectSpread({}, child);

            newChild.id = _UUID["default"].uuid();
            newChild.parentId = newItem.id;
            newChild.parentIndex = hoverIndex;

            if (typeof newChild.field_name !== "undefined") {
              newChild.field_name = newChild.field_name.replace(/[A-Z0-9\-]+/g, "") + _UUID["default"].uuid();
            }

            if (typeof newChild.options !== "undefined") {
              newChild.options = newChild.options.map(function (row) {
                var option = _objectSpread({}, row);
            ...
            _this5.insertCard(newChild, data.length + index);
    ...
    value: function setAsChild(item, child, col, isBusy) {
      var data = this.state.data; // if (this.swapChildren(data, item, child, col)) {
      //   return;
      // }
      ...
      if (oldParent) {
        if (Array.isArray(oldParent.childItems[oldCol])) {
          oldParent.childItems[oldCol] = oldParent.childItems[oldCol].filter(function (x) {
            return x !== child.id;
          });
        } else {
      ...
      var allItemsInRow = item.childItems.flat();
      var toRemove = list.filter(function (x) {
        return allItemsInRow.indexOf(x.id) === -1;
      });
      var newData = data;
    ...
    value: function removeChild(item, col) {
      ...
      var data = this.state.data;
      var oldId = itemId !== null ? itemId : item.childItems[col];
      var oldItem = this.getDataById(oldId);
      ...
      if (oldItem) {
        ...
        if (Array.isArray(item.childItems[col])) {
          item.childItems[col] = item.childItems[col].filter(function (x) {
            return x !== itemId;
          });
        } else {
    ...
    value: function showEditForm() {
      var _this6 = this,
          _this$props$imageMedi,
          _this$props2,
          _this$props$videoMedi,
          _this$props3,
      ...
      var formElementEditProps = {
        ...
        imageMediaSource: (_this$props$imageMedi = (_this$props2 = this.props) === null || _this$props2 === void 0 ? void 0 : _this$props2.imageMediaSource) !== null && _this$props$imageMedi !== void 0 ? _this$props$imageMedi : [],
        videoMediaSource: (_this$props$videoMedi = (_this$props3 = this.props) === null || _this$props3 === void 0 ? void 0 : _this$props3.videoMediaSource) !== null && _this$props$videoMedi !== void 0 ? _this$props$videoMedi : [],
        isNeedAdditionalContext: (_this$props$isNeedAdd = (_this$props4 = this.props) === null || _this$props4 === void 0 ? void 0 : _this$props4.isNeedAdditionalContext) !== null && _this$props$isNeedAdd !== void 0 ? _this$props$isNeedAdd : false
      };
    ...
    value: function render() {
      ...
      var items = data.map(function (item, index) {
        return _this7.getElement(item, index);
      });
