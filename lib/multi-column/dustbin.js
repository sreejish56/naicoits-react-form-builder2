...
var Dustbin = /*#__PURE__*/_react["default"].forwardRef(function (_ref, ref) {
  ...
  var isMultipleComponent = Array.isArray(items[col]);
  var item = isMultipleComponent ? items[col].map(function (row) {
    return getDataById(row);
  }) : getDataById(items[col]);
  ...
  var element = isMultipleComponent ? item.map(function (row) {
    return getElement(row, rest);
  ...
  return connectDropTarget( /*#__PURE__*/_react["default"].createElement("div", {
    ...
    className: isMultipleComponent && (item === null || item === void 0 ? void 0 : item.length) > 1 ? "show-dot-border" : ""
...
var _default = (0, _reactDnd.DropTarget)(function (props) {
  ...
  drop: function drop(props, monitor, component) {
    ...
    var isItemInSameColumn = props.items[props.col] === item.id;

    if (Array.isArray(props.items[props.col]) && props.items[props.col].includes(item.id)) {
      isItemInSameColumn = true;
    } // Do nothing when moving the box inside the same column


    if (props.col === item.col && isItemInSameColumn) return; // Do not allow replace component other than both items in same multi column row
